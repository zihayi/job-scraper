...
- 自动收藏抓取结果到跟踪列表
- 可通过“添加职位”手动录入岗位信息
- 展开岗位后默认只读展示，点击“编辑详情”可修改工作性质、来源链接、职位描述和任职要求
- 为每条收藏设置求职进度（待投递 / 已投递 / 未进面 / 笔试中 / 面试中 / 已offer / 已淘汰）和备注，并自动记录每次状态变更时间
- 标星重点职位并按进度筛选
- 使用 DeepSeek 与已保存岗位进行问答、比较和总结，可切换右侧栏或全页面聊天
- 聊天选择 DeepSeek Reasoner 时，点击“查看思维链”可展开详细分析过程
- 可在设置的“网页代理”分栏启用 Browser Use；启用后，包含明确网址的问题会自动操作页面并整合网页资料
- 导出 Excel 职位名单

收藏数据存在 `saved_jobs.json`（可用 `JOB_STORE_PATH` 改路径）。
聊天历史存在 `chat_history.json`（可用 `CHAT_STORE_PATH` 改路径）。
设置页会显示当前聊天 session 的估算上下文用量。接近模型限制时，程序会在同一聊天界面
自动开启新 session；旧记录继续显示和保存，但不会再发送给模型。
DeepSeek Chat 和 Reasoner 的上下文限制按 `1M tokens` 计算，并为回答预留 8192 tokens。
网页代理默认关闭匿名遥测，仅允许访问问题中明确给出的公网域名，并拦截本机和内网地址。
启用后代理具有输入、键盘、表单、脚本、上传和下载等完整操作权限。默认最多执行 20 步，
可在“网页代理”设置中调整为 1–50 步；单次运行限制在 120 秒内，
请只对可信网站使用。
